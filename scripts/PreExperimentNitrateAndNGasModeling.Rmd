...
$$ log(k_{den}) = \beta_0 + \beta_1log([NO_3^{-1}]) + \epsilon $$
...
data$Vb <- solve(diag(10000,2)) # precisions for multivariate-normal linear regression coefficient prior
...
times <- seq(0, 60*24, length.out = 500)

# Generate data frame of parameter combinations
NO3Pre <- c(100, 200, 300) # Nitrate concentration in SAM before isotope release
deltaNO30 <- c(5000, 10000, 20000) # Isotopic enrichment expressed as a delta value (permil)
percentTracer <- sapply(deltaNO30, calcPercentTracer, Rst = RstN, AFadd = AFadd)
NO3add <- matrix(percentTracer, nrow = 3) %*% matrix(NO3Pre, nrow = 1) # Matrix of added NO3 concentrations for each initial nitrate concentration (columns) and each level of enrichment(rows)
NO30 <- (1+matrix(percentTracer ,nrow = 3)) %*% matrix(NO3Pre, nrow = 1) # Matrix of total NO3 concentration after N15 addition for each initial nitrate concentration (columns) and each level of enrichment(rows)
N15NO30 <- AFadd * NO3add + RToAtomFrac(RstN) * matrix(rep(NO3Pre, each = length(deltaNO30)), nrow = length(deltaNO30)) # Matrix of total N15-NO3 concentration after N15 addition for each initial nitrate concentration (columns) and each level of enrichment(rows)
k2 <- c(0.001, 0.005, 0.01) # Gas exchange constants (min^-1)
params <- data.frame(
...
NO3PIs <- map_dfr(ensembles, getPI, times = times, varName = "N15Nitrate")
...
plotPI(NO3PIs %>% filter(delta == 10000, NO3Pre == 200)) + 
  #facet_wrap(vars(NO3Pre), nrow = 1, labeller = labeller(NO3Pre = NO3PreLabs)) + 
  ylab("[15N-NO3] (uM)")  + 
  ggtitle("(&delta;<sup>15</sup>N-NO<sub>3</sub>)<sub>init</sub> = 10,000&permil; , [NO<sub>3</sub>]<sub>bg</sub> = 200 &mu;g L<sup>-1</sup>") +
  theme(
...
plotPI(NO3PIs %>% filter(delta == 10000, k2 == 0.01)) + 
  facet_wrap(vars(NO3Pre), nrow = 1, labeller = labeller(NO3Pre = NO3PreLabs)) + 
  ylab("[15N-NO3](uM)")  + 
...
# N-gas isotopic composition plots
# Compare results among background nitrate concentrations
plotPI(NGasPIs %>% filter(delta == 10000, k2 == 0.01)) + 
...
# Compare results among gas exchange constants
plotPI(NGasPIs %>% filter(delta == 10000, NO3Pre == 200)) + 
